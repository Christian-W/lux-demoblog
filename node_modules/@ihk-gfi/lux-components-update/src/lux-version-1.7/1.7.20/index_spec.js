...
describe('lux-version-1.7.19', () => {
    ...
    describe('[Rule] checkVersions', () => {
        it('Sollte einen Fehler werfen, wenn Version < n - 1', () => {
            ...
            schematics_1.callRule(index_1.checkVersions(), rxjs_1.of(appTree), context).subscribe((success) => expect(success).toBeUndefined(), (reason) => expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (neuere) Version 1.7.19.'));
        });
        it('Sollte einen Fehler werfen, wenn Version > n - 1', () => {
            test_helper_1.addDependencyToPackageJson(appTree, 'lux-components', '1.7.100');
            schematics_1.callRule(index_1.checkVersions(), rxjs_1.of(appTree), context).subscribe((success) => expect(success).toBeUndefined(), (reason) => expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (ältere) Version 1.7.19.'));
        });
        it('Sollte keinen Fehler werfen, wenn Version === n - 1', () => {
            test_helper_1.addDependencyToPackageJson(appTree, 'lux-components', '1.7.19');
    ...
    describe('[Rule] updatePackageJson', () => {
        beforeEach(() => {
            test_helper_1.addDependencyToPackageJson(appTree, 'lux-components', '1.7.20');
        });
        // Muss über async laufen, da sonst eine package.json geprüft wird, welche bereits wieder resettet wurde
        it('Sollte die Dependency "lux-components" auf Version 1.7.20 setzen', ((done) => __awaiter(this, void 0, void 0, function* () {
            schematics_1.callRule(index_1.updatePackageJson(), rxjs_1.of(appTree), context).subscribe(() => {
                expect(appTree.readContent('/package.json'))
                    .toContain('"lux-components": "1.7.20"');
                done();
