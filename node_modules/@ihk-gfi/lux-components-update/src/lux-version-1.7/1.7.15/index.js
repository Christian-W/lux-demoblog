...
function updatePackageJson() {
    return (tree, context) => {
        logging_1.logInfoWithDescriptor('Aktualisiere LUX-Components Version auf 1.7.15.');
        return util_1.waitForTreeCallback(tree, () => {
            const newDependency = {
 ...
 * @param options
 */
function deleteOldStylesFile(options) {
    logging_1.logInfoWithDescriptor('Lösche luxstyles.scss.');
    return files_1.deleteFile(options, 'src/theming/luxstyles.scss');
}
exports.deleteOldStylesFile = deleteOldStylesFile;
/**
 * Aktualisiert die Dateien im "theming"-Ordner.
 * @param options
 */
function updateThemingFiles(options) {
    logging_1.logInfoWithDescriptor('Aktualisiere die Stylesheets für das Theming.');
    return files_1.moveFilesToDirectory(options, 'files/theming', 'src/theming');
}
exports.updateThemingFiles = updateThemingFiles;
/**
 * Gibt die offen stehenden ToDos (Aufgaben, die der Generator nicht übernehmen konnte) für den User aus.
 */
function todosForUser() {
    return (tree, context) => {
        util_1.runInstallAndLogToDos(context, `Bitte starten Sie ${chalk_1.default.redBright('npm run smoketest')} um möglichen Fehlern vorzugreifen.`, `Weitere Informationen: https://confluence.gfi.ihk.de/display/AF/Update+Guide#UpdateGuide-UmstellungaufVersion1.7.15`);
        return tree;
    };
}
