...
describe('lux-version-1.7.18', () => {
    ...
    describe('[Rule] setupProject', () => {
        it('Sollte Fehler werfen, wenn ein empty Tree genutzt wird', () => {
            expect(() => runner.runSchematic('lux-version-1.7.18', {}, schematics_1.Tree.empty()))
                .toThrowError(schematics_1.SchematicsException);
        });
        it('Sollte Fehler werfen, wenn keine Option "project" gesetzt ist', () => {
            try {
                runner.runSchematic('lux-version-1.7.18', {}, appTree);
            }
            catch (ex) {
    ...
    describe('[Rule] checkVersions', () => {
        it('Sollte einen Fehler werfen, wenn Version < n - 1', () => {
            ...
            schematics_1.callRule(index_1.checkVersions(), rxjs_1.of(appTree), context).subscribe((success) => expect(success).toBeUndefined(), (reason) => expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (neuere) Version 1.7.17.'));
        });
        it('Sollte einen Fehler werfen, wenn Version > n - 1', () => {
            test_helper_1.addDependencyToPackageJson(appTree, 'lux-components', '1.7.100');
            schematics_1.callRule(index_1.checkVersions(), rxjs_1.of(appTree), context).subscribe((success) => expect(success).toBeUndefined(), (reason) => expect(reason.toString()).toContain('Dieser Generator benötigt allerdings die (ältere) Version 1.7.17.'));
        });
    ...
    describe('[Rule] updatePackageJson', () => {
        ...
        it('Sollte die Dependency "lux-components" auf Version 1.7.18 setzen', ((done) => __awaiter(this, void 0, void 0, function* () {
            schematics_1.callRule(index_1.updatePackageJson(), rxjs_1.of(appTree), context).subscribe(() => {
                expect(appTree.readContent('/package.json'))
                    .toContain('"lux-components": "1.7.18"');
                done();
